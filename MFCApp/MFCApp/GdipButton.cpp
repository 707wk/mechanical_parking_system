...
//printf("[%d %d]", rect.Width(), rect.Height());
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			graphics.DrawImage(*m_pStdImage, 0, 0, (int)width, (int)height);        //Ëõ·Å
			...
			if (m_dcStdP.m_hDC == NULL)
			{
				...
				graphics.DrawImage(*m_pStdImage, 1, 1, (int)width, (int)height);
			...
			if(m_dcStdH.m_hDC == NULL)
			{
				...
				//float width = (float)m_pStdImage->m_pBitmap->GetWidth();
				//float height = (float)m_pStdImage->m_pBitmap->GetHeight();
				RectF grect; grect.X=0, grect.Y=0; 
				grect.Width = width; 
				grect.Height = height;
		...
		if( (m_dcAlt.m_hDC == NULL) && m_bHaveAltImage )
		{
			...
			graphics.DrawImage(*m_pAltImage, 0, 0, (int)width, (int)height);
			...
			if( (m_dcAltP.m_hDC == NULL) && m_bHaveAltImage )
			{
				...
				graphics.DrawImage(*m_pAltImage, 1, 1, (int)width, (int)height);
